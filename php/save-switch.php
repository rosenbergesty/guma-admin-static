<?php
  ...
  $containerTwo = '';
  if( isset($_POST['containerTwo']) ){
    $containerTwo = $_POST['containerTwo']; 
  } else {
    $requestBody = file_get_contents('php://input');
    $parsed = json_decode($requestBody, true);
    $containerTwo = $parsed['containerTwo'];
  }
  ...
  // Save Drop
  if(strlen($address) > 0){
    $sql = "UPDATE stops SET timeFulfilled='$time', dateFulfilled='$date', containerNum='$container', containerNum2='$containerTwo', comments='$comments', signature='$signature', image='$pic', status='completed', dropTicket='$drop' WHERE ID='$id'";
    if ($conn->query($sql) === TRUE) {
      ...
      try {
          ...
          $mail->AltBody = 'A drop ticket was generated. See attachment.';
